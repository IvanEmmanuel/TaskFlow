...
class TaskListAPIView(APIView):                             # Estamos creando una vista para nuestra API.   Obtener las tareas y devolverlas.
    ...
    def get(self, request):                                 # Cuando alguien haga una petición HTTP GET a esta vista, ejecuta este método.
        tasks = Task.objects.all()                          # Obtén todas las tareas de la base de datos.
        serializer = TaskSerializer(tasks, many=True)       # many=True ->   Voy a serializar múltiples objetos, no uno solo.
        return Response(serializer.data)                    # serializer.data ->  se encarga de construir la respuesta adecuada para la API.
    
    def post(self, request):
        serializer = TaskSerializer(data=request.data)      # "Serializer, recibe estos datos y dime si son válidos."
        
        if serializer.is_valid():                           # DRF realiza las validaciones del serializer. Si todo está correcto:
...
class TaskDetailAPIView(APIView):
    ...
    def get(self, request, pk):                             # obtiene una pk
        
        task = get_object_or_404(Task, pk=pk)               #  Busca en Task un objeto cuyo pk sea igual al que recibimos en la URL.
        
        serializer = TaskSerializer(task)                   # Tenemos una sola tarea, many=True  no se usa
    ...
    def patch(self, request, pk):
        ...
        serializer = TaskSerializer(
            task,
            data=request.data,
            partial = True                         # Esta actualización es parcial; valida únicamente los campos que estoy enviando.
        )
